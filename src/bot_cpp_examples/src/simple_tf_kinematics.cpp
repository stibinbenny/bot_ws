#include "bot_cpp_examples/simple_tf_kinematics.hpp"
...
SimpleTfKinematics::SimpleTfKinematics(const std::string &name)
  ...
  timer_ = create_wall_timer(
      0.1s, std::bind(&SimpleTfKinematics::timerCallback, this));
...
bool SimpleTfKinematics::getTransformCallback(
  ...
  try {
    requested_transform = tf_buffer_->lookupTransform(
        req->frame_id, req->child_frame_id, tf2::TimePointZero);
